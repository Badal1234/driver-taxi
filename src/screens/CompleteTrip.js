...
export default class DriverCompleteTrip extends React.Component {
    ...
    componentWillMount() {
        ...
        this.setState({
            ...
            region: {
                latitude: allDetails.pickup.latitude,
                longitude: allDetails.pickup.longitude,
                latitudeDelta: 0.9922,
                longitudeDelta: 0.9421,
            },
    ...
    async onPressEndTrip(item) {
        ...
        if(location) {
            ...
            let startLoc = '"'+this.state.rideDetails.pickup.latitude+', '+this.state.rideDetails.pickup.longitude+'"';
    ...
    finalCostStore(item,finalFare,pos,distance,convenience_fees){
        ...
        this.locationAdd(pos).then((response) => response.json()).then((responseJson) => { 
           data.drop = { place:responseJson.results[0].formatted_address,latitude:pos.latitude,longitude:pos.longitude};
           riderData.drop = { place:responseJson.results[0].formatted_address,latitude:pos.latitude,longitude:pos.longitude};
           item.drop = { place:responseJson.results[0].formatted_address,latitude:pos.latitude,longitude:pos.longitude};
           if(data.drop){
    ...
    handleGetDirections(allDetails) {
        const data = {
          ...
          destination: {
            latitude: allDetails.drop.latitude,
            longitude: allDetails.drop.longitude
          },
          params: [
            {
              key: "travelmode",
