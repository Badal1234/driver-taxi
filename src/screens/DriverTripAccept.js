...
export default class DriverTripAccept extends React.Component{
    ...
    componentDidMount() {
    this.getRiders();
  // console.log('gjhghghg')
    RequestPushMsg('cfo_opYaRUqfa8rIHrqrED:APA91bFEJqfrB-dCu1SHfyAYYmWhOXuYx4rFhuyjjWHugElSgK1TGJvk_DAWRHJs4lGkd-v4GASUXVAWUoIczsAY8P20WAg7usboOer9X2TV4IdojHOhFm5nAbB1MBJVo6qlSx68u_U0','shdhdh','5nY61wCOx8YssXj6Pwbv3GxxLbc2')
    }
    ...
    onPressAccept(item) {
        ...
        dbRef.update(data).then(()=>{
            firebase.database().ref('bookings/' + item.bookingId + '/').update(data).then(()=>{
                ...
                // })
                let userDbRef = firebase.database().ref('users/' + item.customer + '/my_bookings/' + item.bookingId + '/');userDbRef.update(riderData);
    ...
    onPressIgnore(item) {
      //  console.log(item.bookingId)
        ...
        .then(()=>{
            let userDbRef = firebase.database().ref('users/' + item.customer + '/my_bookings/' + item.bookingId + '/');
            userDbRef.update({
                status: "CANCELLED",
            });
    ...
    sendPushNotification(customerUID,bookingId,msg){
        ...
        customerRoot.once('value',customerData=>{
            if(customerData.val()){
                ...
                RequestPushMsg(allData.pushToken?allData.pushToken:null,msg,customerUID)
            }
        })
    }
                        ...
                        return (
                            <View style={styles.listItemView}>
                                <View style={styles.mapcontainer}>
                                    ...
                                    >
                                        <Marker
                                            coordinate={{latitude:  item.pickup.latitude, longitude: item.pickup.longitude}}
                                            title={item.pickup.place}
                                            description={languageJSON.pickup_location}                            
                                        />

                                        <Marker
                                            coordinate={{latitude: item.drop.latitude, longitude: item.drop.longitude}}
                                            title={item.drop.place}
                                ...
                                <View style={styles.mapDetails}>
                                    ...
                                    <View style={styles.addressViewStyle}>                            
                                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                            ...
                                            <Text style={styles.addressViewTextStyle}>{item.pickup.place}</Text>
                                        </View>
                                        <View style={styles.fixAdressStyle}>
                                            <View style={styles.redDot}></View>
                                            <Text style={styles.addressViewTextStyle}>{item.drop.place}</Text>
                                        </View>
                                    </View>
